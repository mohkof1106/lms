...
DECLARE
  ...
  v_working_days_per_week INTEGER := 5;
...
BEGIN
  ...
  -- Get settings from company_settings (with defaults)
  SELECT
    COALESCE(working_hours_per_day, 8),
    COALESCE(working_days_per_week, 5)
  INTO v_working_hours_per_day, v_working_days_per_week
  FROM company_settings
  LIMIT 1;

  -- Calculate base working days per year
  v_base_working_days := COALESCE(v_working_days_per_week, 5) * 52;

  -- Count public holidays for current year
  ...
  -- Calculate benefits cost (monthly)
  ...
  SELECT COUNT(*) INTO v_active_employee_count FROM employees WHERE active = true;
  ...
  -- Hourly Cost
  IF COALESCE(v_working_hours_per_day, 8) > 0 THEN
    v_hourly_cost := v_daily_cost / COALESCE(v_working_hours_per_day, 8);
  ELSE
    v_hourly_cost := 0;
  END IF;
  ...
  RETURN QUERY SELECT
    ...
    COALESCE(v_working_hours_per_day, 8),
    COALESCE(v_working_days_per_week, 5);
END;
